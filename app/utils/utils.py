...
def read_file(filename: str) -> List[Tuple[str, float, float]]:
    """Lit un fichier CSV et retourne les données sous forme de liste de tuples.

    Args:
        filename (str): Le nom du fichier CSV à lire.

    Returns:
        List[Tuple[str, float, float]]: Les données sous forme de tuples (nom/action, coût/prix, bénéfice).
    """
    datas: List[Tuple[str, float, float]] = []  # Liste pour stocker les données
